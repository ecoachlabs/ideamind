...
exports.PIIRedactorTool = void 0;
exports.createPIIRedactorTool = createPIIRedactorTool;
...
class PIIRedactorTool {
    ...
    patterns = {
        ...
        phone: /\b(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b|\b\+\d{1,3}[-.\s]?\d{1,14}\b/g,
        // Social Security Numbers (US)
        ssn: /\b\d{3}-\d{2}-\d{4}\b/g,
        // Credit card numbers (Visa, MC, Amex, Discover)
        creditCard: /\b(?:\d{4}[-\s]?){3}\d{4}\b|\b\d{15,16}\b/g,
        // API keys (common patterns)
        apiKey: /\b(?:api[_-]?key|apikey|access[_-]?token|secret[_-]?key|bearer)[:\s=]["']?([A-Za-z0-9_\-]{20,})\b/gi,
        // AWS keys
        awsKey: /\b(?:AKIA[0-9A-Z]{16})\b/g,
        // IP addresses (IPv4)
        ipAddress: /\b(?:\d{1,3}\.){3}\d{1,3}\b/g,
        // Crypto wallet addresses (Bitcoin, Ethereum)
        cryptoWallet: /\b(?:bc1|[13])[a-zA-HJ-NP-Z0-9]{25,62}\b|\b0x[a-fA-F0-9]{40}\b/g,
    ...
    async execute(input) {
        ...
        const { text, redactMode = 'full', sensitivityLevel = 'strict', auditLog = true, } = input;
        try {
            const redactions = [];
     ...
     */
    shouldRedact(piiType, value, sensitivity) {
...
exports.PIIRedactorTool = PIIRedactorTool;
// ============================================================================
// FACTORY
// ============================================================================
function createPIIRedactorTool() {
