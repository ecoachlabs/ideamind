...
class ContradictionScanTool {
    ...
    constructor(phase) {
        ...
        if (phase) {
            this.llm = llm_1.LLMFactory.createProvider(phase, 'validator');
        }
    }
    async execute(input) {
        const startTime = Date.now();
        const { question, answer, phase, runId, dbPool, useLLM = true, } = input;
        try {
            // Step 1: Query existing Knowledge Map for related answers
            const existingKnowledge = await this.queryRelatedKnowledge(dbPool, question, phase, runId);
            console.log(`[ContradictionScan] Found ${existingKnowledge.length} existing K/A pairs to check`);
            ...
            if (useLLM && this.llm) {
                ...
                const llmResult = await this.detectContradictionsWithLLM(question, answer, existingKnowledge);
                conflicts = llmResult.conflicts;
                totalCost = llmResult.cost;
            }
            else {
                // Fallback: Simple rule-based contradiction detection
                conflicts = this.detectContradictionsRuleBased(question, answer, existingKnowledge);
            }
            // Step 3: Calculate consistency score
            const consistencyScore = conflicts.length === 0 ? 1.0 : 0.0;
            console.log(`[ContradictionScan] Consistency: ${consistencyScore}, Conflicts: ${conflicts.length}`);
            return {
                result: {
                    consistencyScore,
     ...
     */
    async queryRelatedKnowledge(dbPool, question, phase, runId) {
     ...
     */
    async detectContradictionsWithLLM(newQuestion, newAnswer, existingKnowledge) {
        if (!this.llm) {
            return { conflicts: [], cost: 0 };
        }
        const prompt = this.buildContradictionPrompt(newQuestion, newAnswer, existingKnowledge);
        try {
...
Be strict but fair. Only flag TRUE contradictions.`,
            ...
            const conflicts = this.parseContradictionResponse(response.content, existingKnowledge);
            return {
                conflicts,
            ...
            return {
                conflicts: this.detectContradictionsRuleBased(newQuestion, newAnswer, existingKnowledge),
                cost: 0,
            };
        }
    }
     ...
     */
    buildContradictionPrompt(newQuestion, newAnswer, existingKnowledge) {
...
Respond ONLY with JSON. No markdown, no explanation.`;
     ...
     */
    parseContradictionResponse(responseText, existingKnowledge) {
        try {
            ...
            return parsed.conflicts.map((c) => {
                const existing = existingKnowledge.find((k) => k.questionId === c.existing_question_id);
                return {
     ...
     */
    detectContradictionsRuleBased(newQuestion, newAnswer, existingKnowledge) {
        const conflicts = [];
        ...
        for (const existing of existingKnowledge) {
            ...
            const questionSimilarity = this.calculateSimilarity(newQuestion.toLowerCase(), existing.question.toLowerCase());
            ...
            if (newNumbers.length > 0 && existingNumbers.length > 0) {
                const hasConflict = this.detectNumericConflict(newNumbers, existingNumbers);
                if (hasConflict) {
                    conflicts.push({
     ...
     */
    detectNumericConflict(newNumbers, existingNumbers) {
...
exports.ContradictionScanTool = ContradictionScanTool;
// ============================================================================
// FACTORY
// ============================================================================
function createContradictionScanTool(phase) {
    return new ContradictionScanTool(phase);
}
//# sourceMappingURL=contradiction-scan.js.map