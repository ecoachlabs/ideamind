...
export class FusionTool implements Tool {
  ...
  async execute(input: ToolInput): Promise<ToolOutput> {
    ...
    const { clusterId, topic, answers, phase } = input as FusionInput;
    ...
    try {
      ...
      const knowledgeFrame = await this.generateKnowledgeFrame(
        synthesis.canonicalAnswer,
        answers,
        phase
      );
  ...
  private async detectConflicts(
    ...
    const llm = LLMFactory.createProvider(phase, 'validator');
...
Respond ONLY with valid JSON. No markdown, no explanation.`;
  ...
  private async synthesizeAnswer(
    ...
    conflicts: Conflict[],
    phase: string
  ): Promise<{ canonicalAnswer: string; evidenceIds: string[] }> {
    const llm = LLMFactory.createProvider(phase, 'answer-agent');
...
Respond ONLY with valid JSON. No markdown, no explanation.`;
   ...
   */
  private async generateKnowledgeFrame(
...
Respond ONLY with valid JSON. No markdown, no explanation.`;
  ...
  private calculateConsensusConfidence(
    ...
    const avgConfidence =
      answers.reduce((sum, a) => sum + (a.confidence || 0.5), 0) / answers.length;
    ...
    const consensus = Math.max(
      0,
      Math.min(1, avgConfidence - conflictPenalty + agreementBoost)
    );
   ...
   */
  private emptyKnowledgeFrame(): KnowledgeFrame {
    return {
      who: '',
      what: '',
      when: '',
...
export interface KnowledgeFrame {
  who: string; // Actor/role
  what: string; // Core concept
  when: string; // Timing
  where: string; // Location/component
  why: string; // Purpose
  how: string; // Method
  metrics: string[]; // Quantitative targets
...
export function createFusionTool(): Tool {
