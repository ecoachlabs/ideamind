...
class ClusterTool {
    ...
    async execute(input) {
        ...
        const { answers, phase, minClusterSize = 2, similarityThreshold = 0.75, } = input;
        try {
            // Step 1: Build similarity matrix
            const similarityMatrix = this.buildSimilarityMatrix(answers);
            // Step 2: Hierarchical clustering
            const clusters = this.hierarchicalClustering(answers, similarityMatrix, similarityThreshold, minClusterSize);
     ...
     */
    buildSimilarityMatrix(answers) {
        const n = answers.length;
        const matrix = Array.from({ length: n }, () => Array(n).fill(0));
        for (let i = 0; i < n; i++) {
            for (let j = i; j < n; j++) {
                const sim = this.cosineSimilarity(answers[i].embedding, answers[j].embedding);
                matrix[i][j] = sim;
                matrix[j][i] = sim;
            }
        }
     ...
     */
    hierarchicalClustering(answers, similarityMatrix, threshold, minSize) {
        // Initialize: each answer in its own cluster
        const clusters = answers.map((answer, idx) => ({
            id: `CLUSTER-${idx + 1}`,
        ...
        while (merged) {
            ...
            for (let i = 0; i < clusters.length; i++) {
                for (let j = i + 1; j < clusters.length; j++) {
                    const sim = this.clusterSimilarity(clusters[i], clusters[j], answers, similarityMatrix);
     ...
     */
    clusterSimilarity(cluster1, cluster2, answers, similarityMatrix) {
     ...
     */
    async labelClusters(clusters, phase) {
...
Respond with ONLY the topic label (2-5 words). No explanation.`;
     ...
     */
    calculateAvgPurity(clusters, similarityMatrix) {
...
exports.ClusterTool = ClusterTool;
// ============================================================================
// FACTORY
// ============================================================================
function createClusterTool() {
