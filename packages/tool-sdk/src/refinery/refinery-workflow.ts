...
import { Pool } from 'pg';
import { NormalizeTool } from '../tools/refine/normalize';
import { FissionTool } from '../tools/refine/fission';
import { EmbedTool, BatchEmbedTool } from '../tools/refine/embed';
import { ClusterTool } from '../tools/refine/cluster';
import { FusionTool, KnowledgeFrame } from '../tools/refine/fusion';
import { OntologyLinkTool } from '../tools/refine/ontologyLink';
import { DedupTool, DedupHandler } from '../tools/refine/dedup';
import { PIIRedactorTool } from '../tools/guard/pii-redactor';

// ============================================================================
// REFINERY WORKFLOW
// ============================================================================

export class RefineryWorkflow {
  private db: Pool;
  private runId: string;
  private phase: string;

  // Tools
  private normalizeTool: NormalizeTool;
  private piiRedactor: PIIRedactorTool;
  private dedupTool: DedupTool;
  private dedupHandler: DedupHandler;
  private fissionTool: FissionTool;
  private ontologyTool: OntologyLinkTool;
  private embedTool: EmbedTool;
  private batchEmbedTool: BatchEmbedTool;
  private clusterTool: ClusterTool;
  private fusionTool: FusionTool;

  // Metrics
  private metrics: RefineryMetrics = {
    inputCount: 0,
    acceptedCount: 0,
    rejectedCount: 0,
    fissionCoverage: 0,
  ...
  constructor(config: RefineryConfig) {
    ...
    // Initialize tools
    this.normalizeTool = new NormalizeTool();
    this.piiRedactor = new PIIRedactorTool();
    this.dedupTool = new DedupTool(this.db);
    this.dedupHandler = new DedupHandler(this.db);
    this.fissionTool = new FissionTool();
    this.ontologyTool = new OntologyLinkTool(this.db);
    this.embedTool = new EmbedTool();
    this.batchEmbedTool = new BatchEmbedTool();
    this.clusterTool = new ClusterTool();
    this.fusionTool = new FusionTool();
  }

  /**
   * Run full Refinery pipeline
   */
  async refine(input: RefineryInput): Promise<RefineryOutput> {
   ...
   */
  private async preprocess(input: RefineryInput): Promise<{
  ...
  private async preprocessText(
    ...
    text: string
  ): Promise<{ text: string; contentHash: string; isDuplicate: boolean }> {
    // Stage 1: Normalize
    ...
    if (dedupResult.result.isDuplicate) {
      ...
      await this.dedupHandler.markAsDuplicate(
        id,
        dedupResult.result.duplicateOf,
        type
      );
      isDuplicate = true;
    }
   ...
   * Stage 5: Fission (question decomposition)
   */
  private async runFission(
    questions: ProcessedQuestion[]
  ): Promise<{ atomicQuestions: AtomicQuestion[]; trees: any[] }> {
    console.log('[Refinery] Stage 5: Fission');

    const atomicQuestions: AtomicQuestion[] = [];
    const trees = [];
    let totalCoverage = 0;
    let treeCount = 0;

    for (const q of questions) {
      if (q.isDuplicate) continue; // Skip duplicates
   ...
   * Stage 6: Entity Linking
   */
  private async runEntityLinking(
   ...
   * Stage 9-10: Fusion + Knowledge Frame Extraction
   */
  private async runFusion(clusters: any[]): Promise<{
    ...
    const canonicalAnswers: CanonicalAnswer[] = [];
    const knowledgeFrames: KnowledgeFrame[] = [];
   ...
   * Stage 11: Version Management
   */
  private async runVersionManagement(fused: any): Promise<void> {
   ...
   * Stage 12: Delta Publishing
   */
  private async publishDeltas(fused: any): Promise<void> {
   ...
   */
  private async getAnswersForCluster(answerIds: string[]): Promise<any[]> {
    const query = `
   ...
   */
  private async storeFissionTree(
    questionId: string,
    tree: any,
    coverage: number
  ): Promise<void> {
   ...
   */
  private async storeEmbedding(embedding: any): Promise<void> {
    const query = `
   ...
   */
  private async storeFusionCluster(cluster: any, fusionResult: any): Promise<void> {
    const query = `
   ...
   * Calculate final metrics
   */
  private calculateFinalMetrics(fission: any, fusion: any): void {
   ...
   */
  private async findExistingVersion(baseId: string): Promise<{
    id: string;
    version: number;
    contentHash: string;
  } | null> {
   ...
   */
  private async createSupersedesEdge(
    oldId: string,
    newId: string,
    version: number,
    reason: string
   ...
   */
  private async storeCanonicalAnswer(
    canonical: any,
    version: number,
    contentHash: string
  ): Promise<void> {
