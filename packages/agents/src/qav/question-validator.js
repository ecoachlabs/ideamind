 ...
 */
class QuestionValidator extends base_agent_1.BaseAgent {
    anthropic;
    dbPool;
    ...
    async execute(input) {
        ...
        try {
            ...
            console.log(`[QV] Validated ${validations.length} pairs: ${acceptedCount} accepted, ${validations.length - acceptedCount} rejected (${durationMs}ms)`);
            return validations;
        }
        catch (error) {
     ...
     */
    async validateBinding(question, answer, existingKMNodes) {
        const issues = [];
        const hints = [];
        ...
        try {
            ...
            if (groundingScore < this.thresholds.grounding) {
                issues.push(`Grounding score ${groundingScore.toFixed(2)} < ${this.thresholds.grounding}`);
            ...
            if (completenessScore < this.thresholds.completeness) {
                issues.push(`Completeness score ${completenessScore.toFixed(2)} < ${this.thresholds.completeness}`);
            ...
            if (specificityScore < this.thresholds.specificity) {
                issues.push(`Specificity score ${specificityScore.toFixed(2)} < ${this.thresholds.specificity}`);
            ...
            if (consistencyScore < this.thresholds.consistency) {
                issues.push(`Consistency score ${consistencyScore.toFixed(2)} < ${this.thresholds.consistency}`);
                hints.push('Resolve contradictions with existing knowledge');
            }
            // Calculate overall score
            const overallScore = (groundingScore + completenessScore + specificityScore + consistencyScore) / 4;
            // Accept if all thresholds met
            const accepted = groundingScore >= this.thresholds.grounding &&
            ...
            if (!accepted && issues.length === 0) {
                issues.push(`Overall score ${overallScore.toFixed(2)} < ${this.thresholds.overall}`);
            }
            return {
                question_id: question.id,
                answer_id: answer.answer_id,
     ...
     */
    async scoreGrounding(answer) {
        if (answer.citations.length === 0) {
            return 0;
        }
        // Calculate average citation confidence
        const avgConfidence = answer.citations.reduce((sum, c) => sum + c.confidence, 0) / answer.citations.length;
     ...
     */
    async scoreCompleteness(question, answer) {
        try {
...
Respond with ONLY a number between 0.0 and 1.0.`;
     ...
     */
    async scoreConsistency(answer, existingKMNodes) {
