...
export class ValidatorHub {
  private phaseConfigs: Map<string, PhaseValidatorConfig>;
  ...
  private initializePhaseConfigs(): void {
    ...
    const defaultRubric: RubricThresholds = {
      grounding: 0.85,
      completeness: 0.80,
      specificity: 0.75,
...
// ============================================================================
// PHASE VALIDATOR CONFIG
// ============================================================================

interface PhaseValidatorConfig {
  systemPrompt: string;
...
interface RubricThresholds {
  grounding: number;      // 0-1: Citation quality
  completeness: number;   // 0-1: Addresses full question
  specificity: number;    // 0-1: Concrete vs hand-waving
  consistency: number;    // 0-1: No conflicts with KM
}
...
class Validator extends BaseAgent {
  ...
  private rubric: RubricThresholds;
  private runId: string;
  private contradictionTool: ContradictionScanTool;
  ...
  constructor(
    ...
    this.llm = LLMFactory.createProvider(config.phase, 'validator');

    // Create contradiction scan tool
    this.contradictionTool = new ContradictionScanTool(config.phase);
  }
  ...
  protected async reason(plan: ExecutionPlan, input: AgentInput): Promise<ReasoningResult> {
    const questionAnswerPairs = (input as any).pairs || [];
    const dbPool: Pool | undefined = (input as any).context?.dbPool;

    // Step 1: Run contradiction detection on all pairs (if dbPool available)
    let contradictionResults: Map<string, any> = new Map();
        ...
        if (contradictionResult) {
          ...
          if (contradictionResult.conflictsDetected) {
            ...
            for (const conflict of contradictionResult.conflicts.slice(0, 3)) {
              binding.hints.push(
                `Conflict with ${conflict.existingQuestionId}: ${conflict.conflictDescription}`
              );
            }
   ...
   */
  private buildValidationPrompt(pairs: any[], contradictionResults?: Map<string, any>): string {
...
${pairs.map((pair, idx) => {
  ...
  const conflictInfo = contradictionResult && contradictionResult.conflictsDetected
    ? `\n- ⚠️ CONFLICTS DETECTED: ${contradictionResult.conflictCount} conflicts found:\n${contradictionResult.conflicts.slice(0, 2).map((c: any) => `    • ${c.conflictDescription}`).join('\n')}`
    : '';
...
Respond ONLY with JSON. No markdown, no explanation.`;
   ...
   */
  private parseBindings(responseText: string, pairs: any[]): Array<{
    question_id: string;
    answer_id: string;
    ...
    try {
      ...
      return parsed.bindings.map((b: any) => ({
   ...
   */
  private fallbackValidation(pairs: any[]): ReasoningResult {
