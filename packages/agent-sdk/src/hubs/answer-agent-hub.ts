...
export class AnswerAgentHub {
  private phaseConfigs: Map<string, PhaseAnswerConfig>;
...
class AnswerAgent extends BaseAgent {
  ...
  private evidenceSources: string[];
  ...
  constructor(
    ...
    this.llm = LLMFactory.createProvider(config.phase, 'answer-agent');
  }
  ...
  private extractArtifactsContent(input: AgentInput): string {
    ...
    const relevantArtifacts = input.artifacts.filter((artifact) =>
      this.evidenceSources.some((source) =>
        artifact.type.toLowerCase().includes(source.toLowerCase())
      )
    );
   ...
   */
  private buildAnswerPrompt(questions: any[], artifactsContent: string): string {
...
Respond ONLY with JSON. No markdown, no explanation.`;
   ...
   */
  private parseAnswers(responseText: string, questions: any[]): Array<{
    question_id: string;
    answer: string;
    evidence_ids: string[];
    assumptions: string[];
    ...
    try {
      ...
      return parsed.answers.map((a: any) => ({
   ...
   */
  private checkGrounding(answers: any[]): boolean {
    // Need improvement if:
    // 1. Any answer has no evidence_ids
    // 2. Any answer has low confidence
    // 3. Many assumptions
    const poorlyGrounded = answers.filter(
      (a) => a.evidence_ids.length === 0 || a.confidence < 0.6
    );
   ...
   */
  private fallbackAnswers(questions: any[]): ReasoningResult {
