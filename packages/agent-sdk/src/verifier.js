...
class Verifier {
     ...
     */
    async compare(baseline, toolResult, config) {
        ...
        const baselineTotal = dimensions.reduce((sum, d) => sum + d.baselineScore * d.weight, 0);
     ...
     */
    async scoreWithLLM(content, apiKey) {
...
Respond ONLY with a JSON object in this exact format (no additional text):
{
  ...
  "relevance": <score 0-100>
        ...
        const validateScore = (score) => Math.max(0, Math.min(100, score));
        return {
