...
import { ToolRegistryClient } from '@ideamine/tool-sdk';
...
export class Analyzer {
  private registryClient: ToolRegistryClient;

  constructor(private config: AgentConfig) {
    const registryUrl = process.env.TOOL_REGISTRY_URL || 'http://localhost:3000';
    this.registryClient = new ToolRegistryClient(registryUrl);
  }
   ...
   */
  private async identifyCandidateTools(
    plan: ExecutionPlan,
    currentResult: ReasoningResult,
    input: AgentInput
  ): Promise<Array<{
    ...
    try {
      ...
      const relevantTools = approvedTools.filter(tool => {
        ...
        const isRelevant = this.isToolRelevantForTask(
          tool.category,
          plan.phaseId,
          currentResult.needsImprovement
        );
      ...
      const candidates = relevantTools.map(tool => {
        ...
        const estimatedImprovement = this.estimateToolImprovement(
          tool.category,
          confidenceGap
        );
   ...
   */
  private estimateToolImprovement(category: string, confidenceGap: number): number {
  ...
  private calculateVoI(
    ...
    const remainingBudgetRatio =
      (input.budget.maxCostUsd - input.budget.currentCostUsd) / input.budget.maxCostUsd;
